...
import os
'''
使用方法：python regression.py ak sk bucket container_name
使用前提：
1.已知 avatest 账号的 ak/sk
2.在 avatest 账号的 bucket 中，已经包含 folder_1w 文件夹，且其中有超过5000个文件
3.在 avatest 账号的 bucket 中，已经有多个文件和文件夹
...
def execute_ignore_output(title,cmd,i):
    ...
    returncode = subprocess.call([cmd],shell = True,stdout = devNull)
    if returncode == 0:
        print "Success to "+title+" !"
    else:
        print "Fail to "+title+" !"
...
def main():
    ...
    cmd9 = docker+"mkdir -p /test-alluxio-fuse"+"&&"+docker+"/opt/alluxio/integration/fuse/bin/alluxio-fuse \
    mount /test-alluxio-fuse /a/b/1381102889/alluxio-test"
    cmd10 = docker+"/opt/alluxio/integration/fuse/bin/alluxio-fuse umount /test-alluxio-fuse"+"&&"+docker+ "/opt/alluxio/integration/fuse/bin/alluxio-fuse \
    mount /test-alluxio-fuse /a/b/1381102889/alluxio-test "
    ...
    cmd15 = docker+"/opt/alluxio/bin/alluxio fs mv /a/b/1381102889/alluxio-test/test-write /a/b/1381102889/alluxio-test/write-test" \
    +"&&"+docker+"/opt/alluxio/bin/alluxio fs mv /a/b/1381102889/alluxio-test/write-test /a/b/1381102889/alluxio-test/test-write"
    ...
    cmd19 = docker+"curl -O http://devtools.qiniu.com/linux/amd64/qrsctl?ref=developer.qiniu.com" \
            +"&&"+docker+"mv qrsctl\?ref\=developer.qiniu.com qrsctl" \
    ...
    title = [title0,title1,title2,title3,title4,title5,title6,title7,title8,title9,title10,title11, \
